...
const add_order_list = (menu_list) => {
  ...
  const orders = menu_list.map((order, index) => {
    ...
    menu_qty.classList.add("menu_price");
    menu_qty.innerText = order.to_price;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (order_article) {
    order_article.addEventListener("click", (e) => {
      const target = e.target;
      if (target.tagName === "DIV" && target.className.include("menu_delete")) {
  ...
  if (pay_box) {
    pay_box.addEventListener("click", (e) => {
      ...
      if (button.className.include("btn_cash")) {
        alert("현금결제");
      } else {
        alert("카드결제");
      }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (button.tagName === "BUTTON") {
      if (button.className.includes("btn_cash")) {
        ...
        document.querySelector("spna.pay_qty").innerText = "카드결제";
        modal.style.display = "flex";
      }
